...
**默认 paper 模拟交易，不触碰真实资金；live 实盘需显式配置 `TRADE_MODE=live` + `TRADE_BACKEND` 才会真实报单。live 链路自 2026-08-27 起已接入硬风控断路器（下单频率 + 单笔金额占比，账户看不清时拒单），但单日亏损检查尚未覆盖——启用实盘仍等同自行承担剩余风险。**
...
| **特别监管类 · 实盘交易** | `live` 实盘模式为**特别监管类**功能：需使用者本人显式配置并自行承担全部风险。当前版本 live 链路已覆盖**下单频率 + 单笔金额占比**两项硬风控（账户信息不可用时拒单），但**单日亏损检查未覆盖**，故仍不构成完整的风控保障。 |
...
2. 确认已理解 live 链路仅覆盖「下单频率 + 单笔金额占比」两项硬风控，**单日亏损检查未覆盖**，需在券商侧或外部另行设置止损风控；
