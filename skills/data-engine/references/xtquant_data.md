---
created: 2026-08-18T11:48:26 (UTC +08:00)
tags: []
source: https://dict.thinktrader.net/nativeApi/start_now.html
author: 
---

> **职责拆分说明**：本文件仅保留 **XtQuant.XtData 行情模块**（行情/财务/基础行情/板块行业等**数据获取**类内容），服务于 `data-engine`（数据获取）技能。
>
> **实盘交易接口**（`XtQuant.Xttrade 交易模块`：报单/撤单/查询/信用交易/约券/回调推送）已拆分至 `execution-monitor-engine/references/xtquant_trading.md`，由 `execution-monitor-engine`（实盘交易监控）技能维护。
